...
int main(void)
{
  ...
  // Counter and String Buffer
  int blinked = 0;
  char buffer[1024];
  while (1)
  {
	  // LED Print Statment
	  sprintf(buffer, "LED HAS BLINKED %d TIMES\r\n", blinked);

	  // UART Transmission LED Set High
	  HAL_UART_Transmit(&huart2, (uint8_t *)buffer, sizeof(buffer)-1, 10);
	  HAL_GPIO_WritePin(LED_PIN_GPIO_Port, LED_PIN_Pin, GPIO_PIN_SET);

	  // Delay 200ms (192 to account for inaccuracy)
	  HAL_Delay(192);
	  blinked++;
	  // LED set Low
	  HAL_GPIO_WritePin(LED_PIN_GPIO_Port, LED_PIN_Pin, GPIO_PIN_RESET);
	  // Delay 300ms (278 to account for inaccuracy)
	  HAL_Delay(278);
	  // Clear Buffer
